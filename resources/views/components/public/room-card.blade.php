...
<a href="{{ route('rooms.show', [$locale, $room['slug']]) }}"
   class="group relative isolate flex min-h-[19rem] flex-col justify-end overflow-hidden bg-espresso-900 text-white shadow-sm transition duration-500 hover:shadow-xl sm:min-h-[20rem] lg:min-h-[22rem]">
    ...
    <div class="absolute inset-0 -z-10 bg-gradient-to-t from-espresso-900 via-espresso-900/50 to-espresso-900/10"></div>
    ...
    {{-- Content --}}
    <div class="relative flex items-end justify-between gap-3 p-5 sm:gap-4 sm:p-7">
        <div class="min-w-0 flex-1">
            <h3 class="font-display text-2xl leading-tight sm:text-3xl lg:text-4xl">{{ $room->tr('name') }}</h3>
            <p class="mt-2 line-clamp-2 max-w-sm text-sm leading-relaxed text-white/70 sm:text-base">{{ $room->tr('description') }}</p>

            <div class="mt-4 flex divide-x divide-white/15 sm:mt-5">
                <div class="pr-4 sm:pr-6">
                    <p class="text-[10px] font-semibold uppercase tracking-[.12em] text-copper-400/80">{{ $en ? 'Single' : 'Individual' }}</p>
                    <p class="mt-0.5 font-medium text-white sm:mt-1 sm:text-lg">{{ number_format($room->price, 0, ',', ' ') }} Kz</p>
                </div>
                <div class="pl-4 sm:pl-6">
                    <p class="text-[10px] font-semibold uppercase tracking-[.12em] text-copper-400/80">{{ $en ? 'Double' : 'Duplo' }}</p>
                    <p class="mt-0.5 font-medium text-white sm:mt-1 sm:text-lg">{{ number_format($room->price_double, 0, ',', ' ') }} Kz</p>
                </div>
            </div>
        </div>
